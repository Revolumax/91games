...
export default function Quienes() {
  ...
  const fetchMinecraftUsername = async (uuid: string) => {
    const url = `https://cors-anywhere.herokuapp.com/https://api.mojang.com/user/profiles/${uuid}/names`;
    ...
    const fetchNames = async () => {
      const updatedMembers = await Promise.all(
        teamMembers.map(async (member) => {
          if (!member.hasUUID) {
            const name = prompt(`Ingrese el nombre para ${member.name}:`);
            return { ...member, name: name || member.name }; // Permite ingresar nombre manualmente
          }
          ...
          const currentName = await fetchMinecraftUsername(uuid);
